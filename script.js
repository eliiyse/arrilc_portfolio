...
backToTop.addEventListener('click', () => window.scrollTo({ top: 0, behavior: 'smooth' }));
 ...
 $('#invertBtn').addEventListener('click', () => document.body.classList.toggle('inverted'));
...
let isMuted = false, audioContext;
...
function playSound(type) {
  ...
  if (type === 'hover') { osc.frequency.value = 880; gain.gain.setValueAtTime(0.01, audioContext.currentTime); gain.gain.exponentialRampToValueAtTime(0.001, audioContext.currentTime + 0.05); }
  else { osc.frequency.value = 440; gain.gain.setValueAtTime(0.02, audioContext.currentTime); gain.gain.exponentialRampToValueAtTime(0.001, audioContext.currentTime + 0.1); }
...
const sectionMap = [
  ...
  { id: 'contactSection', link: document.querySelector('.nav-link[href="#contactSection"]') }
];
...
sectionMap.forEach(s => { const el = document.getElementById(s.id); if (el) scrollSpy.observe(el); });
...
lightboxPrev.addEventListener('click', () => { currentImageIndex = (currentImageIndex - 1 + galleryItems.length) % galleryItems.length; updateLightbox(); });
lightboxNext.addEventListener('click', () => { currentImageIndex = (currentImageIndex + 1) % galleryItems.length; updateLightbox(); });

const folderData = {
  quiz: {
    name: 'Quiz', files: 1,
    projects: [
      { name: 'QUIZ 1', desc: 'Online assessment / perfect score of 20/20 on IT fundamentals.',
        img: 'https://drive.google.com/thumbnail?id=1s3DTeDG95bFvjOkBrdXYLhXecp-dEcwD&sz=w1920',
        tag: 'Online Quiz', lightbox: true }
    ]
  },
  exams: { name: 'Exams', files: 0, projects: [] },
  laboratory: {
    name: 'Laboratory', files: 2,
    projects: [
      { name: 'VM SETUP: UBUNTU INSTALLATION', desc: 'Oracle VM VirtualBox setup — Ubuntu 26.04 Desktop, 4GB RAM, 4 CPU cores, 100GB dynamic disk.',
        img: 'https://drive.google.com/thumbnail?id=1Evq-TPJ3IwnErv39EKgTemqeO0MIRYUq&sz=w1000',
        tag: 'Virtualization', pdf: true,
        link: 'https://drive.google.com/file/d/1mtuyw7eCEgUSbftoBHFXAFwSZSVsD5mQ/preview' },
      { name: 'LAB ACTIVITY 2', desc: 'Hands-on laboratory exercise — practical output documenting a step-by-step process with screenshots and documentation.',
        img: 'https://drive.google.com/thumbnail?id=1AP3dxDd631Yg-ZxRgs7QBpJP1IRnv1wv&sz=w1000',
        tag: 'Laboratory', pdf: true,
        link: 'https://drive.google.com/file/d/1HemXkwY64wFxBd8ptjLzFx-j8taI2Zem/preview' }
    ]
  },
  projects: {
    name: 'Projects', files: 6,
    projects: [
      { name: 'BLOOMING INTERACTIVE FLOWER', desc: 'TouchDesigner / generative flower that blooms through hand interaction.',
        img: 'https://drive.google.com/thumbnail?id=18jhfNMeuhyleGUlN_bqmGNjDkcHMwjZ_&sz=w1000', tag: 'TouchDesigner', live: false },
      { name: 'WISPY PARTICLES', desc: 'TouchDesigner / flowing particle system playing in real time.',
        img: 'https://drive.google.com/thumbnail?id=1jyeOr9mEq_lw6DMx7TcLPpmh1fn9G99h&sz=w1000', tag: 'TouchDesigner', live: false },
      { name: 'FINGER TRACKING', desc: 'Computer vision / hand landmark detection and skeletal visualization.',
        img: 'https://drive.google.com/thumbnail?id=1jU7oDp2Q497vkYnbfcEZG9NizZuJDR2p&sz=w1000', tag: 'Computer Vision', live: false },
      { name: 'CALCUNIELI', desc: 'Web app / a fun and simple calculation project.',
        img: 'https://drive.google.com/thumbnail?id=1FqYhQkZsBwxTAZ1DJD43wogK8mucVmrJ&sz=w1000', tag: 'Web App', live: true,
        link: 'https://calcunieli.vercel.app/' },
      { name: 'POMODORO', desc: 'Web app / focus timer with session tracking, streak counter, and break management.',
        img: 'https://drive.google.com/thumbnail?id=1AF634MPS43E_RIQ_67bzOJtfeOxr_1_F&sz=w1000',
        tag: 'Web App', live: true,
        link: 'https://arrilrj.github.io/pomodoro/' },
      { name: 'TTPD STORYTELLING', desc: 'Web project / interactive storytelling experience with dynamic visuals and narrative flow.',
...
const modalBody = $('#modalBody');
 ...
 $$('.folder-item').forEach(item => {
  ...
  item.addEventListener('mouseleave', () => $('#previewPanel').classList.remove('active'));
  ...
  item.addEventListener('click', () => {
    ...
    if (data.projects.length > 0) {
      const singleClass = data.projects.length === 1 ? ' modal-projects-single' : '';
      ...
      data.projects.forEach(p => {
        ...
        const pdfBadge = p.pdf ? '<div class="modal-project-pdf-badge"><i data-lucide="file-text"></i> PDF</div>' : '';
        const arrow = isClickable ? '<div class="modal-project-arrow"><i data-lucide="arrow-up-right"></i></div>' : '';
        let link, target;
        if (p.live || p.pdf) { link = p.link; target = 'target="_blank" rel="noopener noreferrer"'; }
        else if (p.lightbox) { link = '#'; target = `data-lightbox-src="${p.img}"`; }
        else { link = '#'; target = 'onclick="return false;" style="cursor: default;"'; }
        let visualHtml;
        if (p.pdf && !p.img) {
